...
With no action, perform readiness only and offer local testing followed by relevant actions below. On the readiness tool, `--unattended`, `--auto`, and `--jdi` only add its implemented unattended behaviour, including configured runtime startup. They do not authorize review, fixes, commits, pushes, publication, approval or merging. An explicit action request controls subsequent work. An existing JDI issue-implementation authorization can carry through this handoff for completion and token-usage audits only; apply the audit publication authorization in `prs-finish`. Do not infer that authorization from readiness flags.
