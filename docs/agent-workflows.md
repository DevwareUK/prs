...
1. Use `superpowers:brainstorming` to inspect repository behavior, ask questions and settle the requirements.
2. Write and self-review the specification, use `superpowers:writing-plans` to write and self-review the implementation plan from it, and prepare the exact issue draft, linked set or original refinement target without requesting intermediate approval.
3. Show the exact target/content and both reviewed artifacts together. Obtain one explicit user approval that accepts the specification and plan and authorizes the displayed creation or refinement write plus publication of both managed comments. A question, qualification, scope change, content change or target change is not approval; update every affected artifact, show the complete revised packet and request one fresh approval.
4. Check both displayed, approved files exist and contain non-empty Markdown. Use `prs tool issue create` with both `--spec-file` and `--plan-file` for new issues, or `prs tool issue publish-artifacts <number>` with both files for existing-issue refinement.
