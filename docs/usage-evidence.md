...
Capture requires an existing real run directory and a regular source file. Source-file symlinks and aliased outputs are rejected. JSON arrays and non-Copilot sources use the existing 64 MiB whole-file limit. Copilot file-exporter JSONL is read synchronously in fixed chunks only through the source size observed at open; unrelated sessions are discarded as each complete line is parsed. One complete record may not exceed 8 MiB and no more than 100,000 matching records are retained. Appends beyond the snapshot are available at the next checkpoint. A per-output lock prevents simultaneous writers; after an interrupted process, inspect that no capture is running before removing its `.lock` file. Unknown usage remains unavailable/partial, never zero.

### Host connection

For the macOS Copilot app, the skill installer and interactive setup can configure an opt-in persistent local export plus an exact-session bridge. Use `prs skills install copilot` and accept the prompt, or `prs setup --skills all --copilot-telemetry enable`. Restart the app normally afterward. When the canonical capture command runs from an active local Copilot session, the managed `preToolUse` hook records only the session ID, canonical repository root, observation time, schema version, and fixed purpose. The binding is accepted only for the same repository and for at most 60 seconds. See [setup, disable, data minimization, and preservation rules](github-copilot.md#optional-macos-app-usage-tracking).
...
| Copilot | Explicit `--session` / `--source`; otherwise an existing connected evidence binding, `PRS_USAGE_SESSION_ID` / `PRS_USAGE_SOURCE`, `COPILOT_OTEL_FILE_EXPORTER_PATH`, then the opted-in fresh managed session binding and verified exporter. | OTLP JSON chat spans or individual exported span records, with matching `gen_ai.conversation.id`. Parent `invoke_agent` spans and metric snapshots are not added. |

Use an explicitly known session ID, never the newest file or a guessed ID. For Claude Code, SessionStart hook input provides `session_id` and `transcript_path`; an existing user-managed hook can expose these as `PRS_USAGE_SESSION_ID` and `PRS_USAGE_SOURCE` through `CLAUDE_ENV_FILE`. This is optional setup, not a hook installed by PRS. For opted-in local Copilot app/CLI use, the bare command consumes the fresh managed binding and verified exporter. Explicit `--session` and `--source` values remain authoritative. Existing connected evidence remains fixed; environment variables precede managed fallback. Missing, stale, invalid, or multiple fresh bindings stay unavailable. Keep content capture disabled. Hosted Copilot coding-agent evidence remains unsupported. See [Claude hooks](https://code.claude.com/docs/en/hooks), [Claude monitoring](https://code.claude.com/docs/en/monitoring-usage) and [Copilot hooks](https://docs.github.com/en/copilot/reference/hooks-reference) for native setup. Do not launch an extra session just to validate capture.
