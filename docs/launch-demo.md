...
| 4 | `prs pr address-comments <pr-number>` | Pull selected GitHub review comments into a focused local fix handoff. | This is the first guided write path, but it stays tightly scoped to chosen review tasks. | Requires a clean working tree and GitHub PR access. Writes a timestamped `.prs/runs/` directory for the active Codex session without launching another runtime, committing, or pushing. |
| 5 | `prs pr add-tests <pr-number>` | Pull selected items from the managed AI test suggestions comment into a focused local testing handoff. | Proves that AI can move from review signal to concrete automated test work with preserved task context. | Same local trust boundary as comment fixing, but the source of truth is the managed test-suggestions comment. Writes a dedicated `.prs/runs/` directory for the active Codex session without launching another runtime. |
...
- `.prs/runs/<timestamp>-pr-<pr-number>-address-comments/`
- `.prs/runs/<timestamp>-pr-<pr-number>-add-tests/`
...
- Beta: `prs issue batch <number> <number> [...number]`, `prs codex pr prepare-review <pr-number>`, `prs feature-backlog`
