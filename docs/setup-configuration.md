...
Setup also writes PRS-managed AI profile defaults into `.prs/config.json`. The default profiles are `premium` (`gpt-5.5`, `high` thinking) and `standard` (`gpt-5.4-mini`, `medium` thinking). Default role routing uses `premium` for planner/reviewer work and `standard` for implementer/tester work.
...
- `ai.profiles`: named model and thinking-level profiles used by Codex delegation and separate Codex work. Each profile contains `model` and `thinking`. Supported thinking values are `none`, `minimal`, `low`, `medium`, `high`, and `xhigh`. `prs setup` writes `premium` as `gpt-5.5` with `high` thinking and `standard` as `gpt-5.4-mini` with `medium` thinking.
- `ai.roles`: workflow role routing to profile names. Supported role keys are `planner`, `implementer`, `reviewer`, and `tester`. Each value must reference an `ai.profiles` entry. `prs setup` routes `planner` and `reviewer` to `premium`, and `implementer` and `tester` to `standard`.
...
- if no `ai.profiles` config is present, `prs` uses the default `premium` and `standard` profiles
- if no `ai.roles` config is present, `prs` uses the default planner/reviewer-to-premium and implementer/tester-to-standard routing
