...
| `prs issue estimate <number>` | Compatibility shell estimate for the issue's managed `<!-- prs:issue-plan -->` comment. The primary active-Codex route is `/prs issue <number> estimate`, which runs `prs tool issue estimate-context <number> --json`, has Codex create the structured estimate from the plan, then publishes it with `prs tool issue publish-estimate <number> --file <path> --json`. If no managed plan exists, the estimate context output names the expected `<!-- prs:issue-plan -->` marker and asks you to publish a managed plan comment or run `prs issue plan <number>` first. |
...
- `prs tool issue estimate <issue-number> --json` remains a deterministic compatibility estimate for non-Codex automation
