...
- `--media-manifest <path>` can be used with `prs issue draft --draft-file`, `prs tool issue create --draft-file`, or `prs audit publish` to add a Visual References/Visual Evidence section. The manifest is a JSON array or `{ "media": [...] }`; items provide exactly one of `url` or `path`, optional `kind` (`image` or `video`), optional `caption`, and optional `alt`. Supported local files are limited to 25 MB for images and 100 MB for videos. URL images are embedded, URL videos are linked, and local files are validated then rendered as local path references because the CLI cannot upload binary attachments through a stable GitHub issue/PR API.
