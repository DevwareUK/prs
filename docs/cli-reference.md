...
- `prs tool issue publish-estimate <issue-number> --file <path> --json`: publish a Codex-authored estimate JSON artifact to the issue audit comment
...
- `/prs issue <number> estimate` is plan-first and Codex-mediated: `estimate-context` prepares the managed plan/profile/verification context, Codex writes the estimate JSON, and `publish-estimate` publishes or updates the issue audit `Estimate` section using the same compact table layout as the token-usage ledger
...
- `prs tool issue create --json` accepts `--spec-file` and `--plan-file` for approved local artifacts. When those artifacts exist and are non-empty, it publishes the managed marker comments and returns them in `managedComments`; `managedCommentHints` lists only marker comments that still remain to make the created issue estimate-ready. After a managed plan is published, it attempts a non-blocking deterministic estimate publication and reports the outcome in `estimatePublicationHints`. When `--run-dir` contains `codex-token-usage.json`, it publishes the dedicated managed `<!-- prs:token-usage -->` comment before spec/plan comments and reports it in `tokenUsageComments`.
- Managed issue workflows track available token usage in one managed `<!-- prs:token-usage -->` comment per issue. `/prs create`, `/prs issue <number> refine`, `/prs issue <number> estimate`, and `/prs issue <number>` implementation/finalization runs keep per-run `codex-token-usage.json` evidence under `.prs/runs` and publish the table with `prs tool token-usage publish --issue <number> --file <path> --json`. A token artifact may contain one entry or an `entries` array for multiple Codex goals/sessions in the same run; each entry should carry a stable `id` for its workflow phase plus current run/session. The managed GitHub comment is the merge source of truth: each publish fetches the current comment, upserts only the entry IDs from the current artifact, preserves rows from other machines/runs, and rewrites the visible table plus hidden structured ledger data. In active Codex app sessions, `/prs create` and `/prs issue <number> refine` create or reuse a planner-scoped Codex goal, verify `get_goal` can see the active goal before drafting/refining continues, and call `get_goal` again before writing `codex-token-usage.json`; token totals from `tokensUsed` or `usage.totalTokens` are recorded as tracked usage, and unavailable rows are reserved for cases where no active goal is visible or the goal usage fields are genuinely unavailable. The ledger table is a concise overview with phase, role, model provenance, status, total tokens, rough estimated cost, elapsed time, and capture time; raw per-run details stay in the local artifact. Estimated ledger costs use configured model rates and blend ratios when model and total-token data are available. `prs audit publish --section token-usage` remains a compatibility wrapper for the managed token-usage comment and must not publish raw JSON.
