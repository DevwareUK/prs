...
| `/prs create observability` | Managed Codex shortcut for DSM observability findings. The skill infers `--site` from the current repository when possible, defaults to `--env prod` and `--since 24h`, runs `dsm grafana triage` to produce JSON and Markdown artifacts under `.prs/runs`, then passes the JSON artifact to `prs issue draft --observability-findings <path>`. This is a command wrapper around the artifact contract; PRS still does not query Grafana, Prometheus, Loki, or Faro directly. |
