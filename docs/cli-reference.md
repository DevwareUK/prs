...
- `prs issue estimate <number>` is read-only and bounded; it estimates implementation effort from the managed plan rather than drafting/refinement effort, and its rough USD cost ranges are planning aids rather than exact billing data
...
- `prs tool issue estimate <issue-number> --json` returns the same estimate as structured JSON for active Codex sessions and automation, including per-profile cost ranges, the priced token range/formula, and cost-basis metadata
