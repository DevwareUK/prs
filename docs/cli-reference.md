...
- `/prs create observability [--site <site>] [--env <env>] [--since <duration>]`: run DSM Grafana triage, save findings under `.prs/runs`, and import the JSON artifact into the approval-gated issue draft flow
- `prs issue draft --observability-findings <path>`: import DSM observability findings into approval-gated local issue draft artifacts
...
- `/prs cleanup worktrees`: safe cleanup route for PRS-managed worktrees
...
- `prs tool issue context <issue-number> --json`: return read-only repository identity, issue body/comments, managed spec/plan presence, and linked pull requests for refinement or planning in the active coding-agent session
...
- `prs tool issue estimate-context <issue-number> --json`: return the managed plan, verification commands, and output schema for an active Codex session to create a model-based estimate without PRS-owned model profiles
- `prs tool issue publish-estimate <issue-number> --file <path> --json`: publish a Codex-authored estimate JSON artifact to the issue token telemetry comment
- `prs tool issue publish-artifacts <issue-number> --spec-file <path> --plan-file <path> --json`: validate caller-approved non-empty Markdown and create or update the existing issue's managed spec and plan comments without generating or modifying artifact text
- `prs tool token-usage publish (--issue <number>|--pr <number>) --file <path> --json`: publish a run-local `codex-token-usage.json` artifact to the dedicated managed `<!-- prs:token-usage -->` issue or PR comment
- `prs tool issue create (--draft-file <path>|--issue-set <path>) --json [--run-dir <path>] [--spec-file <path>] [--plan-file <path>] [--media-manifest <path>]`: deterministically create GitHub issues from approved local issue draft artifacts, publish supplied managed token-usage/spec/plan artifacts, return any remaining managed comment hints for estimate readiness, and report token usage in `tokenUsageComments` when `codex-token-usage.json` exists in the run directory
...
- `prs tool pr publish-review <pr-number> --report <path> --comments <path> [--review-status <request-changes|comment|approve>] [--unattended|--auto|--jdi] --json`: publishes the completed local Codex PR review report to the managed PR audit comment and posts a GitHub review event with high-confidence line-linked review comments on changed lines; unattended aliases add visible automation framing
...
- `prs tool worktrees cleanup [--apply] --json`: report PRS-managed worktrees, explain whether each candidate is safe to remove, and remove only safe candidates when `--apply` is present
...
Legacy issue support commands are kept in the detailed reference below for older automation, GitHub Action and manual recovery support, and explicit escape hatches. They are intentionally absent from the command-tier overview when there is a skill-first or deterministic handoff path.
...
| `prs issue draft --observability-findings <path>` | DSM observability import. Reads a DSM-owned `dsm grafana triage` JSON findings artifact, validates `version: 1`, skips malformed individual findings with diagnostics, keeps only actionable findings at medium severity or higher for the active GitHub repository, skips likely duplicates by finding ID, fingerprint, or suggested title, and writes local draft/spec/plan artifacts under `.prs/runs/<timestamp>-issue-draft/`. One selected finding follows the single issue draft preview; multiple selected findings follow the linked issue-set preview. The normal approve/modify/cancel gate still applies before GitHub issue creation. PRS does not query Grafana, Prometheus, Loki, or Faro directly. |
| `/prs create observability` | Managed Codex shortcut for DSM observability findings. The skill infers `--site` from the current repository when possible, defaults to `--env prod` and `--since 24h`, runs `dsm grafana triage` to produce JSON and Markdown artifacts under `.prs/runs`, then passes the JSON artifact to `prs issue draft --observability-findings <path>`. Pasted notes, logs, attachments, or old draft text do not replace the DSM findings artifact as the issue source. This is a command wrapper around the artifact contract; PRS still does not query Grafana, Prometheus, Loki, or Faro directly. |
...
| `prs issue refine <number>` | Interactive existing-issue refinement flow. Fetches the current issue body plus comments, resumes the saved runtime session when that session is still tracked locally, otherwise asks whether to specify changes to the original requirements, defaults to no, only asks for change text when you answer yes, and starts a fresh refinement run, writes resumable state to `.prs/issues/<number>/refine-session.json` plus run artifacts to `.prs/runs/<timestamp>-issue-refine-<number>/`. The runtime may write refinement questions, one refined Markdown draft, or a multi-issue set in `.prs/runs/<timestamp>-issue-refine-<number>/issue-set.json`. Unresolved questions are posted as a source issue comment and stop the run. Approved single drafts publish managed `<!-- prs:issue-spec -->` and `<!-- prs:issue-plan -->` comments plus a final confidence comment on the source issue without overwriting its body. Multi-issue sets are kept on disk with guidance to split work explicitly outside refinement. If GitHub authentication is unavailable, the refined draft or set is kept on disk instead of being applied. Refine metadata records the source issue token-usage ledger target and run-local `codex-token-usage.json` path. |
...
| `prs issue estimate <number>` | Compatibility shell estimate for the issue's managed `<!-- prs:issue-plan -->` comment. It prints estimated implementation token ranges, rough USD cost ranges, confidence, drivers, warnings, and a recommendation. Cost ranges use PRS model-rate defaults, an 80% input / 20% output token split, and any `.prs/config.json` `ai.costEstimates` overrides; structured JSON identifies the priced token range and formula for each per-profile cost range. The primary active-Codex route is `/prs issue <number> estimate`, which runs `prs tool issue estimate-context <number> --json`, has Codex create the structured estimate from the plan, then publishes it with `prs tool issue publish-estimate <number> --file <path> --json`. If no managed plan exists, the estimate context output names the expected `<!-- prs:issue-plan -->` marker and asks you to publish a managed plan comment or run `prs issue plan <number>` first. |
| `prs issue prepare <number>` | GitHub Action and manual recovery support. Preflights the configured forge, verification command, and `baseBranch`, creates a missing managed issue plan comment before writing the runtime snapshot, checks the plan's `### Likely files` against files changed by open pull requests, prompts in interactive terminals when overlap remains, prepares the issue branch from the selected base, and then prints machine-readable JSON describing the run. Prefer `/prs issue <number> --jdi` or `prs tool issue ready <number> --unattended --json` from active Codex sessions. |
| `prs issue prepare <number> --mode github-action` | GitHub Action support. Same preparation flow, including missing-plan creation, but writes prompt instructions tailored for non-interactive GitHub Actions runs. |
| `prs issue finalize <number>` | GitHub Action and manual recovery support. Generates a deterministic local proposed commit message from the current repository diff, including included untracked files, lets you preview, edit, or skip it, and creates the commit only after confirmation. It never uses the configured text provider, so it does not require `OPENAI_API_KEY` just to finalize Codex-produced changes. It does not push or open a pull request. |

Important behavior:

- `prs issue draft --observability-findings <path>`, `prs issue draft --draft-file <path>`, `prs issue draft --issue-set-file <path>`, `prs issue draft --runtime`, `prs issue plan <number> [--refresh]`, `prs issue prepare <number>`, `prs issue finalize <number>`, and full `prs issue <number>` runs print an advanced workflow notice before execution
- `prs issue <number> <number> ...` and `prs issue batch ...` print a beta workflow notice before execution
- `/prs issue <number> estimate` is plan-first and Codex-mediated: `estimate-context` prepares the managed plan/profile/verification context, Codex writes the estimate JSON, and `publish-estimate` publishes or updates the issue's managed `<!-- prs:token-usage -->` token telemetry comment with estimates in a separate compact table from actual usage rows
- `prs tool issue estimate <number> --json` is the deterministic compatibility estimate; its rough USD cost ranges are planning aids rather than exact billing data
...
- `prs tool issue create --json` accepts `--spec-file` and `--plan-file` for approved local artifacts. When those artifacts exist and are non-empty, it publishes the managed marker comments and returns them in `managedComments`; `managedCommentHints` lists only marker comments that still remain to make the created issue estimate-ready. After a managed plan is published, it attempts a non-blocking deterministic estimate publication to the token telemetry comment and reports the outcome in `estimatePublicationHints`. When `--run-dir` contains `codex-token-usage.json`, it publishes the dedicated managed `<!-- prs:token-usage -->` comment before spec/plan comments and reports it in `tokenUsageComments`.
- Managed issue workflows track available token telemetry in one managed `<!-- prs:token-usage -->` comment per issue. `/prs create`, `/prs issue <number> refine`, `/prs issue <number> estimate`, and `/prs issue <number>` implementation/finalization runs keep per-run `codex-token-usage.json` evidence or estimate artifacts under `.prs/runs`. Actual usage publishes with `prs tool token-usage publish --issue <number> --file <path> --json`; estimates publish with `prs tool issue publish-estimate <number> --file <path> --json`. A token artifact may contain one entry or an `entries` array for multiple Codex goals/sessions in the same run; each entry should carry a stable `id` for its workflow phase plus current run/session. The managed GitHub comment is the merge source of truth: each publish fetches the current comment, upserts only the entry IDs from the current artifact, preserves rows from other machines/runs, and rewrites separate visible usage and estimate tables plus hidden structured ledger data. Estimate publication adds forecast rows without deleting actual usage rows. In active Codex app sessions, `/prs create` and `/prs issue <number> refine` create or reuse a planner-scoped Codex goal, verify `get_goal` can see the active goal before drafting/refining continues, and call `get_goal` again before writing `codex-token-usage.json`; token totals from `tokensUsed` or `usage.totalTokens` are recorded as tracked usage, and unavailable rows are reserved for cases where no active goal is visible or the goal usage fields are genuinely unavailable. Raw per-run details and estimate rationale stay in the local artifact and hidden structured ledger data. Estimated ledger costs use configured model rates and blend ratios when model and total-token data are available. `prs audit publish --section token-usage` remains a compatibility wrapper for the managed token telemetry comment and must not publish raw JSON.
- PR-related workflows track PR-lifetime token usage with the same renderer. `prs tool pr ready <number> --json`, `prs tool pr review <number> --json`, `prs pr address-comments <number>`, `prs pr add-tests <number>`, and `prs pr fix-tests <number>` return or persist `tokenUsage` metadata pointing at `.prs/runs/.../codex-token-usage.json` and `prs tool token-usage publish --pr <number> --file <path> --json`.
- Managed active-Codex `/prs issue <number> --unattended`, `--auto`, and `--jdi` runs track actual Codex usage with a prs-owned run artifact when usage data is available. Codex goals are optional telemetry sources, not the lifecycle authority for completion. The skill records `codex-token-usage.json` in the issue run directory, includes workflow identity, capture phase, and audit publication state when available, and updates the original issue's token telemetry ledger during `/prs finish` before reporting the managed skill run complete. When `/prs finish` opens or updates the pull request for an issue run, implementation token usage must publish to the original issue ledger before any PR-lifetime token telemetry is published to the pull request. Model names come from actual Codex session metadata when available; PRS does not provide role/profile model fallbacks. This is actual available run/session telemetry, not exact billing.
- Unattended issue runs also maintain `.prs/runs/.../issue-orchestration-state.json` as the resumable lifecycle ledger. Active Codex `/prs issue <number> --jdi` flows should continue from that ledger through PR readiness/review, review publication, review-comment fixes, bounded CI waiting/fixes, and final audit; blocked or skipped stages should be reported with their recorded summary and retry command. After `/prs finish` creates or updates the pull request, the active issue pipeline runs `prs tool pr ready <pr-number> --unattended --json`, then `prs tool pr review <pr-number> --unattended --json`, writes the returned review report and comment artifacts, and publishes them with `prs tool pr publish-review <pr-number> --report <path> --comments <path> --unattended --json` before moving to comment fixes, bounded CI handling, and final audit. GitHub Actions `pr-review` output is a separate automation signal and does not satisfy the active Codex review stage.
- Managed active-Codex `/prs issue <number> --unattended`, `--auto`, and `--jdi` runs track actual Codex usage with a prs-owned run artifact when usage data is available. Codex goals are optional telemetry sources, not the lifecycle authority for completion. The skill records `codex-token-usage.json` in the issue run directory, includes workflow identity and available estimate-style capture metadata, records capture phase and audit publication state when available, and updates the original issue's token telemetry ledger during `/prs finish` before reporting the managed skill run complete. When `/prs finish` opens or updates the pull request for an issue run, implementation token usage must publish to the original issue ledger before any PR-lifetime token telemetry is published to the pull request. Model names come from actual Codex session metadata when available; PRS does not provide role/profile model fallbacks. This is actual available run/session telemetry, not exact billing; estimate forecast rows publish to a separate table in the same managed comment.
- Unattended issue runs also maintain `.prs/runs/.../issue-orchestration-state.json` as the resumable lifecycle ledger. Active Codex `/prs issue <number> --jdi` flows should continue from that ledger through PR readiness/review, review publication, review-comment fixes, bounded CI waiting/fixes, final audit, and ready-for-review promotion; blocked or skipped stages should be reported with their recorded summary and retry command. After `/prs finish` creates or updates the pull request, the active issue pipeline runs `prs tool pr ready <pr-number> --unattended --json`, then `prs tool pr review <pr-number> --unattended --json`, writes the returned review report and comment artifacts, and publishes them with `prs tool pr publish-review <pr-number> --report <path> --comments <path> --review-status <request-changes|comment|approve> --unattended --json` before moving to comment fixes, bounded CI handling, final audit, and marking a successful draft PR ready for review. GitHub Actions `pr-review` output is a separate automation signal and does not satisfy the active Codex review stage.
...
- `prs issue draft` requires one of `--draft-file`, `--issue-set-file`, `--observability-findings`, or explicit `--runtime`
...
- `prs issue <number>`, `prs issue <number> --unattended`, `prs issue prepare <number>`, and each child of a multi-issue run create a missing managed issue plan comment before the issue snapshot is written; if a direct managed plan comment already exists, the latest edited direct managed comment is used unchanged
...
- approving a single refined draft publishes managed specification and plan comments on the source issue without overwriting the original issue body
- generated multi-issue refinements are kept on disk instead of creating linked issues; split work should be decided explicitly outside refinement
...
- after an approved Superpowers-backed draft or refinement, a non-empty `superpowers-plan.md` creates or updates the managed `<!-- prs:issue-plan -->` issue plan comment; prs then attempts a non-blocking deterministic estimate publication for that plan, while missing or empty plan artifacts are logged and do not block issue creation or refinement
...
- `prs tool issue estimate-context <issue-number> --json` and `prs tool issue publish-estimate <issue-number> --file <path> --json` are the active-Codex estimate handoff commands
- `prs tool issue estimate <issue-number> --json` remains a deterministic compatibility estimate for non-Codex automation, including per-profile cost ranges, the priced token range/formula, and cost-basis metadata
...
| `prs tool pr publish-review <pr-number> --report <path> --comments <path> [--review-status <request-changes\|comment\|approve>] [--unattended\|--auto\|--jdi] --json` | Requires the completed local Codex PR review report and structured comments JSON, validates comments against the captured review diff, keeps only high-confidence comments anchored to changed right-side diff lines, adds hidden `prs:pr-review-inline` metadata, skips duplicate active PRS-authored findings, publishes the Markdown report through the managed audit comment, and creates a GitHub pull request review using `REQUEST_CHANGES`, `COMMENT`, or `APPROVE`. If `--review-status` is omitted for compatibility, the tool infers `request-changes` when high-confidence inline findings remain and `comment` otherwise. When GitHub rejects an `APPROVE` review with validation status 422, prs retries the GitHub review as `COMMENT` and includes fallback details in the JSON result because the authenticated account usually cannot approve its own pull request. Unattended aliases apply visible automation framing to the audit comment, review body, and inline review comments. |
