...
- Managed active-Codex `/prs issue <number> --unattended`, `--auto`, and `--jdi` runs track actual Codex goal usage when goal tools are available. The skill records `codex-token-usage.json` in the issue run directory and includes a `token-usage` section in the final PR audit comment during `/prs finish`. This is actual session usage when available, not the forecast returned by `prs issue estimate <number>`.
