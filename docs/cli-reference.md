...
- `prs tool issue create (--draft-file <path>|--issue-set <path>) --json [--run-dir <path>] [--spec-file <path>] [--plan-file <path>] [--media-manifest <path>]`: deterministically create GitHub issues from approved local issue draft artifacts, publish supplied managed spec/plan artifacts, return any remaining managed comment hints for estimate readiness, and return token-usage audit publication hints when `codex-token-usage.json` exists in the run directory
