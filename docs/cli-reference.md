...
- `prs tool issue create --json` accepts `--spec-file` and `--plan-file` for approved local artifacts. When those artifacts exist and are non-empty, it publishes the managed marker comments and returns them in `managedComments`; `managedCommentHints` lists only marker comments that still remain to make the created issue estimate-ready.
- Managed active-Codex `/prs issue <number> --unattended`, `--auto`, and `--jdi` runs track actual Codex usage with a prs-owned run artifact when usage data is available. Codex goals are optional telemetry sources, not the lifecycle authority for completion. The skill records `codex-token-usage.json` in the issue run directory, includes available estimate-style model profile metadata such as `standard (gpt-5.4-mini, medium thinking)`, records capture phase and audit publication state when available, and includes a `token-usage` section in the original issue completion audit comment during `/prs finish` by publishing it with `prs audit publish --issue <number> --file <path> --section <name>` before reporting the managed skill run complete. This is actual session usage when available, not the forecast returned by `prs issue estimate <number>`.
