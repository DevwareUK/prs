...
For Codex issue implementation runs, `codex-token-usage.json` may appear in the issue run directory. It records whether actual token usage was `tracked`, `partial`, or `unavailable`, plus the captured Codex goal usage fields exposed by the active Codex environment.
