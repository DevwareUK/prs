...
Planner workflows also track token usage when available. `/prs create` records create-run usage under the issue draft run directory and publishes a `token-usage` audit section to the created issue after creation. The deterministic `prs tool issue create --json` result includes `auditPublicationHints` for `codex-token-usage.json` when that artifact exists under the supplied `--run-dir`. `/prs issue <number> refine` records refinement usage under `.prs/runs/<timestamp>-issue-refine-<number>/` metadata and publishes the usage audit to the source issue when it posts questions or publishes managed refinement artifacts. This planner usage is separate from implementation-run usage and from `prs issue estimate <number>` forecasts.
...
When the active Codex app exposes goal tools, `/prs issue <number> --unattended`, `/prs issue <number> --auto`, and `/prs issue <number> --jdi` should create or reuse an issue-scoped goal such as `Complete PRS issue #<number>: <title>`. During `/prs finish`, capture the latest available usage and estimate-style model profile metadata into `codex-token-usage.json` in the issue run directory and include a concise `token-usage` section in the original issue completion audit comment published with `prs audit publish --issue <number> --file <path> --section <name>`. Model metadata should mirror `prs issue estimate` profile formatting, such as `standard (gpt-5.4-mini, medium thinking)` for the default implementer profile or `premium (gpt-5.5, high thinking)` for premium-profile work. The `.prs/runs/.../codex-token-usage.json` artifact is the workflow source of truth. Codex goal data may populate it, but the final issue audit must be published before the skill marks the goal or managed run complete. This is actual tracked session usage when available; it is separate from `prs issue estimate <number>`, which remains a forecast from the managed plan.
...
For Codex planner and implementation runs, `codex-token-usage.json` may appear in the run directory. It records whether actual token usage was `tracked`, `partial`, or `unavailable`, workflow identity, available workflow role/profile/tier/model/thinking metadata, capture phase, audit publication state, and usage fields exposed by the active Codex environment. Create and refine planner artifacts use the issue draft or issue refine run directory and publish `token-usage` to the created or source issue; implementation artifacts use the issue implementation run directory and publish the completion `token-usage` section to the original issue, not the pull request audit. The `.prs/runs/.../codex-token-usage.json` artifact is the workflow source of truth. Codex goal data may populate it, but the relevant issue audit must be published before the skill marks the goal or managed run complete.
