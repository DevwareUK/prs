...
const MANAGED_COMMENT_AUTOMATION_NOTE =
...
function formatSuperpowersPlanArtifactComment(planMarkdown: string): string {
  return formatManagedIssueComment(ISSUE_PLAN_COMMENT_MARKER, planMarkdown);
}

function formatSuperpowersSpecArtifactComment(specMarkdown: string): string {
  return formatManagedIssueComment(ISSUE_SPEC_COMMENT_MARKER, specMarkdown);
}

function formatManagedIssueComment(marker: string, markdown: string): string {
  const trimmed = markdown.trim();
  const body = trimmed.startsWith(marker)
    ? trimmed.slice(marker.length).trimStart()
    : trimmed;
  const framedBody = body.startsWith(MANAGED_COMMENT_AUTOMATION_NOTE)
    ? body
    : `${MANAGED_COMMENT_AUTOMATION_NOTE}\n\n${body}`;
...
async function publishIssueRefinementCompleteComment(options: {
  ...
  const comment = await options.forge.createIssuePlanComment(
    ...
    [
      ...
      "prs automation note: refinement is complete. The settled specification and implementation plan have been attached to this issue as managed comments, so development can start from those artifacts.",
      "",
    ].join("\n")
  );
  logSuperpowersPlanPublicationMessage(
...
function formatIssueRefinementQuestionsComment(questionsMarkdown: string): string {
  return formatManagedIssueComment(
