...
export async function offerCopilotTelemetry(input: {
  ...
  const answer = (await input.promptForLine("Enable local usage tracking for the Copilot app? This adds macOS login settings and a private exact-session hook for local-only export, disables content capture, and also affects Copilot CLI/future processes. Restart the app afterward. [y/N]: ")).trim().toLowerCase();
