...
const GIT_AI_WORKFLOW_MARKER = "# Generated by git-ai setup";
const GIT_AI_ACTION_REF = "main";
const GIT_AI_ACTION_REPOSITORY = "DevwareUK/git-ai";
...
const RECOMMENDED_GITHUB_WORKFLOWS: readonly GitHubWorkflowTemplate[] = [
  {
    fileName: "git-ai-pr-review.yml",
    id: "pr-review",
    label: "PR review",
  },
  {
    fileName: "git-ai-pr-assistant.yml",
    id: "pr-assistant",
    label: "PR assistant",
  },
  {
    fileName: "git-ai-test-suggestions.yml",
    id: "test-suggestions",
...
function detectRuntimeType(
  ...
  return {
    ...
    source: "git-ai default runtime",
    warnings: [
      claudeAvailability.available
        ? "Codex is not available on PATH yet, so interactive git-ai workflows would fail until you install it. Claude Code is available if you prefer to switch the runtime explicitly."
...
function isLegacyManagedGitHubWorkflowContent(
  ...
  return (
    ...
    content.includes(`${GIT_AI_ACTION_REPOSITORY}/${actionPath}@`)
  );
}
...
function findMissingGitHubWorkflowIds(repoRoot: string): GitHubWorkflowId[] {
  return RECOMMENDED_GITHUB_WORKFLOWS.filter(
    (workflow) => !existsSync(getGitAiWorkflowPath(repoRoot, workflow.fileName))
...
function renderPrReviewWorkflow(): string {
  return [
    ...
    "        run: |",
    "          DIFF_FILE=\"$RUNNER_TEMP/git-ai-pr-review.diff\"",
...
function renderPrAssistantWorkflow(): string {
  return [
    ...
    "          HEAD_SHA: ${{ github.event.pull_request.head.sha }}",
    "        run: |",
    "          DIFF_FILE=\"$RUNNER_TEMP/git-ai-pr-assistant.diff\"",
    "          git diff --unified=3 \"$BASE_SHA\" \"$HEAD_SHA\" > \"$DIFF_FILE\"",
    "          echo \"diff_file=$DIFF_FILE\" >> \"$GITHUB_OUTPUT\"",
    "          if [ -s \"$DIFF_FILE\" ]; then",
    "            echo \"has_diff=true\" >> \"$GITHUB_OUTPUT\"",
    ...
    "          HEAD_SHA: ${{ github.event.pull_request.head.sha }}",
    "        run: |",
    "          COMMIT_MESSAGES_FILE=\"$RUNNER_TEMP/git-ai-pr-assistant-commits.txt\"",
...
function renderTestSuggestionsWorkflow(): string {
  return [
    ...
    "          HEAD_SHA: ${{ github.event.pull_request.head.sha }}",
    "        run: |",
    "          DIFF_FILE=\"$RUNNER_TEMP/git-ai-test-suggestions.diff\"",
    "          git diff --unified=3 \"$BASE_SHA\" \"$HEAD_SHA\" > \"$DIFF_FILE\"",
    "          echo \"diff_file=$DIFF_FILE\" >> \"$GITHUB_OUTPUT\"",
    "          if [ -s \"$DIFF_FILE\" ]; then",
    "            echo \"has_diff=true\" >> \"$GITHUB_OUTPUT\"",
...
async function collectSetupAnswers(
  ...
  const updateAgents = await promptYesNo(
    promptForLine,
    agentsFileExists
      ? "Add or update a minimal git-ai managed scaffold in AGENTS.md"
...
export async function runSetupCommand(options: {
  ...
  for (const skippedPath of workflowInstallResult.skipped) {
    console.log(
      `Skipped ${skippedPath} because it is not managed by git-ai setup.`
    );
  }
