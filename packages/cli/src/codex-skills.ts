...
export const PRS_CODEX_SKILLS: ManagedCodexSkill[] = [
  ...
  },
  {
    ...
    body: [
      ...
      "5. Before publishing the final audit, call `get_goal` when goal tools are available, capture the active model identifier/source when the Codex environment or configured role exposes it, write the latest Codex usage snapshot to the issue run directory as `codex-token-usage.json`, and include a concise `token-usage` section in the PR audit artifact. If usage or model metadata is unavailable, record that status or note instead of blocking completion.",
