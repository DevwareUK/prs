...
const SHARED_WORKFLOW_CONTRACT = [
  ...
  "- Read `.prs/config.json` for the repository's subagent preference and runtime/provider settings. Workflow roles such as planner, implementer, reviewer, and tester remain internal workflow labels and do not select PRS-managed model definitions.",
  ...
  "- If `ai.codex.preferSubagents` is explicitly enabled, treat it as the repository's standing request to delegate suitable independent tasks to subagents when the subagent tool is available.",
  ...
  "- If `ai.codex.preferSubagents` is disabled or omitted, do not treat the repository as having standing subagent delegation consent.",
...
function renderPrPublishReviewToolCommand(unattended = false): string {
  return unattended
    ? "prs tool pr publish-review <number> --report <reportFilePath> --comments <commentsFilePath> --review-status <request-changes|comment|approve> --unattended --json"
    : "prs tool pr publish-review <number> --report <reportFilePath> --comments <commentsFilePath> --review-status <request-changes|comment|approve> --json";
}
...
export const PRS_CODEX_SKILLS: ManagedCodexSkill[] = [
  {
    ...
    body: [
      ...
      "- `/prs issue <number> finish`: finish work with the issue context preserved; after a pull request exists, standalone guided runs offer the next `/prs pr` step for that pull request, while `/prs issue <number> --jdi` runs return to the issue pipeline for PR readiness, active Codex review, comments, CI, final audit, and ready-for-review promotion.",
      ...
      `- \`/prs pr <number> review\`: run \`${renderPrReviewToolCommand()}\`, read the returned \`promptFilePath\` and \`contextFilePath\`, inspect the prepared checkout in this active Codex session, write the final report to the returned \`reportFilePath\`, write inline review candidates to the returned \`commentsFilePath\`, choose review status \`request-changes\`, \`comment\`, or \`approve\`, present a concise approval summary, and run \`${renderPrPublishReviewToolCommand()}\` only after the user approves posting to GitHub. Do not edit code, commit, push, resolve comments, or post directly to GitHub outside the approved publish tool.`,
      `- \`/prs pr <number> review --unattended\` (aliases: \`--auto\`, \`--jdi\`): run \`${renderPrReviewToolCommand(true)}\`, write the report and inline review candidates, choose review status \`request-changes\`, \`comment\`, or \`approve\`, then publish with \`${renderPrPublishReviewToolCommand(true)}\`. This unattended GitHub-visible output must keep visible automation framing.`,
  ...
  },
  {
    ...
    body: [
      ...
      "5. Before publishing the final audit, call `get_goal` when goal tools are available and write the latest available Codex usage snapshot to the issue run directory as `codex-token-usage.json` with a stable entry `id` for the workflow phase plus current run/session. If one run has multiple Codex goals or sessions, store them as a version 1 `token-usage-ledger` with an `entries` array. Prefer actual active Codex session model metadata when available; do not invent or use PRS-owned role/profile model fallbacks. If usage or model metadata is unavailable, record that status or note instead of blocking completion. Treat Codex goals as telemetry sources, not as the lifecycle authority for audit publication.",
  ...
  },
  {
    ...
    body: [
      ...
      "Immediately before writing create-run token usage, call `get_goal` again when goal tools are available and map exposed usage fields into `codex-token-usage.json`: use `tokensUsed` or `usage.totalTokens` for total tokens, `timeUsedSeconds` or `usage.timeUsedSeconds` for elapsed time, the goal `threadId` as `sessionId` or `goal.threadId` when exposed, and an ISO `capturedAt`. The publisher enriches actual Codex session model metadata when available; do not invent or fall back to PRS-owned model profiles. Give each entry a stable `id` for the workflow phase plus current run/session, and if one run has multiple Codex goals or sessions, store them as a version 1 `token-usage-ledger` with an `entries` array. Use `status: \"tracked\"` when token totals are present, `status: \"partial\"` when a goal exists but only some fields are exposed, and `status: \"unavailable\"` only when `get_goal` is unavailable, no active goal exists, or the goal exposes no usable usage fields. Do not write an unavailable artifact merely because model metadata is unavailable.",
      ...
      "When recording model metadata, prefer the actual active Codex session model when available; do not invent or use PRS-owned role/profile model fallbacks.",
  ...
  },
  {
    ...
    body: [
      ...
      `For \`/prs:pr <number> review --unattended\` (aliases: \`--auto\`, \`--jdi\`), run \`${renderPrReviewToolCommand(true)}\`; write the report and inline review candidates, choose review status \`request-changes\`, \`comment\`, or \`approve\`, then publish with \`${renderPrPublishReviewToolCommand(true)}\`. This unattended GitHub-visible output must keep visible automation framing.`,
