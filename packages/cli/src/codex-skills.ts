...
export const PRS_CODEX_SKILLS: ManagedCodexSkill[] = [
  ...
  },
  {
    ...
    body: [
      ...
      "5. Before publishing the final audit, call `get_goal` when goal tools are available, capture the workflow role plus estimate-style profile/model/thinking tuple such as `standard (gpt-5.4-mini, medium thinking)` or `premium (gpt-5.5, high thinking)` when the Codex environment or configured role exposes it, write the latest Codex usage snapshot to the issue run directory as `codex-token-usage.json`, and include a concise `token-usage` section in the original issue completion audit artifact. If usage or model metadata is unavailable, record that status or note instead of blocking completion. Treat Codex goals as telemetry sources, not as the lifecycle authority for audit publication.",
      "6. Publish final verification, PR state, and token-usage notes to the original issue with `prs audit publish --issue <number> --file <path> --section <name>`.",
  ...
  },
  {
    ...
    body: [
      ...
      "Record planner token usage for the create run under the issue-draft run directory as `codex-token-usage.json`; if usage is unavailable, record a partial or unavailable artifact with notes.",
      ...
      "After the create tool returns created issue numbers, publish a token-usage audit section to each created issue with `prs audit publish --issue <number> --file <path> --section token-usage` before reporting the create workflow complete.",
  ...
  },
  {
    ...
    body: [
      ...
      "For `/prs:issue <number> refine`, record planner token usage under the issue-refine run directory as `codex-token-usage.json` and publish the token-usage audit section to the source issue when refinement posts questions, publishes managed spec/plan comments, or posts the final confidence comment.",
      ...
      "Keep Codex token usage artifacts under the issue run directory as `codex-token-usage.json`; include the workflow role, profile/tier name, model, thinking level, and source when the active Codex environment, configured role, or operator-provided run notes expose them. Mirror the `prs issue estimate` profile format: `<profile> (<model>, <thinking> thinking)`. Use `status: \"tracked\"` when `get_goal` exposes usage, `status: \"partial\"` when only some fields are available, and `status: \"unavailable\"` when the environment cannot expose usage.",
  ...
  },
  {
    ...
    body: [
      ...
      "Publish final verification, PR state, and token-usage notes to the original issue with `prs audit publish --issue <number> --file <path> --section <name>`.",
