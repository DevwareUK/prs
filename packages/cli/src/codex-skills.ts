...
export const PRS_CODEX_SKILLS: ManagedCodexSkill[] = [
  ...
  },
  {
    ...
    body: [
      ...
      "When a PR tool result or metadata includes `tokenUsage`, write the Codex usage snapshot to the returned `artifactFile` as `codex-token-usage.json` when active session usage is available, preserve the workflow name/role and PR target, and publish it with `prs tool token-usage publish --pr <number> --file <path> --json` at the listed publication point. If usage or model metadata is unavailable, record `partial` or `unavailable` telemetry without blocking PR readiness, review publication, fixes, commits, or guarded pushes.",
