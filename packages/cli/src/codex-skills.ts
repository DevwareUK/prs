...
export const PRS_CODEX_SKILLS: ManagedCodexSkill[] = [
  ...
  },
  {
    ...
    body: [
      ...
      "5. Before publishing the final audit, call `get_goal` when goal tools are available, capture the workflow role plus estimate-style profile/model/thinking tuple such as `standard (gpt-5.4-mini, medium thinking)` or `premium (gpt-5.5, high thinking)` when the Codex environment or configured role exposes it, write the latest Codex usage snapshot to the issue run directory as `codex-token-usage.json` with a stable entry `id` for the workflow phase plus current run/session, and update the original issue's `token-usage` ledger. If one run has multiple Codex goals or sessions, store them as a version 1 `token-usage-ledger` with an `entries` array. Prefer the actual active Codex session model when available; configured role/profile metadata is fallback provenance and must not be reported as the actual model. If usage or model metadata is unavailable, record that status or note instead of blocking completion. Treat Codex goals as telemetry sources, not as the lifecycle authority for audit publication.",
  ...
  },
  {
    ...
    body: [
      ...
      "Record planner token usage for the create run under the issue-draft run directory as `codex-token-usage.json`; give each entry a stable `id` for the workflow phase plus current run/session, and if one run has multiple Codex goals or sessions, store them as a version 1 `token-usage-ledger` with an `entries` array. If usage is unavailable, record a partial or unavailable artifact with notes.",
