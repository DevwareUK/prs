...
export const PRS_CODEX_SKILLS: ManagedCodexSkill[] = [
  {
    ...
    body: [
      ...
      "- `/prs issue <number> estimate`: run `prs tool issue estimate <number> --json`, show the returned token ranges, rough USD cost ranges with their priced token range/formula, cost-basis metadata, confidence, drivers, warnings, and recommendation, and stop. This route is read-only: do not launch a runtime, edit files, commit, push, or publish GitHub comments.",
  ...
  },
  {
    ...
    body: [
      ...
      "5. Before publishing the final audit, call `get_goal` when goal tools are available, capture the workflow role plus estimate-style profile/model/thinking tuple such as `standard (gpt-5.4-mini, medium thinking)` or `premium (gpt-5.5, high thinking)` when the Codex environment or configured role exposes it, write the latest Codex usage snapshot to the issue run directory as `codex-token-usage.json`, and include a concise `token-usage` section in the PR audit artifact. If usage or model metadata is unavailable, record that status or note instead of blocking completion.",
