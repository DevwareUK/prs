...
PR-related workflows use the same shared token audit renderer for PR-lifetime `token-usage` ledgers. `prs tool pr ready <pr-number> --json`, `prs tool pr review <pr-number> --json`, `prs pr address-comments <pr-number>`, `prs pr add-tests <pr-number>`, and `prs pr fix-tests <pr-number>` return or persist `tokenUsage` metadata that points at the run-local `.prs/runs/.../codex-token-usage.json` artifact and the matching `prs audit publish --pr <number> --file <path> --section token-usage` publication target. PR token ledgers use the same compact table columns as issue ledgers, but render as `Codex token usage ledger for PR #<number>`. If usage or actual model metadata is unavailable, PR workflows should record partial or unavailable telemetry without blocking readiness, review publication, fixes, commits, or guarded pushes.
