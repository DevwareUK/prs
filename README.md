...
- `ai.issue.useCodexSuperpowers`: repository default for Superpowers-backed issue draft and refine workflows. When `true`, `prs issue draft` and `prs issue refine <number>` add Codex Superpowers-specific instructions if the launched runtime is Codex and Superpowers is available in the current Codex installation. Final issue drafts still use the normal `.prs/issues/` or refine-run draft paths, while intermediate Superpowers spec and plan artifacts stay inside the current `.prs/runs/<timestamp>-issue-draft/` or `.prs/runs/<timestamp>-issue-refine-<number>/` directory. `prs setup` detects local Codex Superpowers availability and writes this preferred flag automatically. Default: `false`.
...
- if Superpowers-backed issue workflows are enabled but Superpowers is unavailable when `prs issue draft` or `prs issue refine <number>` runs, `prs` prints a clear fallback message and uses the standard prompt instead of failing
...
- `ai.issue.useCodexSuperpowers` affects `prs issue draft` and `prs issue refine <number>` and is ignored unless the launched runtime is Codex; legacy `ai.issueDraft.useCodexSuperpowers` is still accepted when the broader setting is absent
...
- if Superpowers-backed issue workflows are enabled but local Codex Superpowers is no longer available, `prs issue draft` and `prs issue refine <number>` print a fallback notice and continue with the standard prompt
