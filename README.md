...
Managed `/prs issue <issue-number> --unattended`/`--jdi` Codex skill runs also track actual Codex goal usage when the active Codex environment exposes goal tools. The skill starts or reuses an issue-scoped `create_goal` objective, records the latest `get_goal` usage snapshot under the issue run directory as `codex-token-usage.json`, and includes a concise `token-usage` section in the final pull request audit comment published during `/prs finish`. If usage is unavailable, the workflow records that status without blocking verification, commit, push, or PR creation.
