...
`prs tool issue estimate <issue-number> --json` remains a deterministic compatibility estimate for automation that cannot ask Codex to reason over the plan. New Codex-facing integrations should prefer `estimate-context` plus `publish-estimate`.

Managed `/prs issue <issue-number> --unattended`/`--jdi` Codex skill runs also track actual Codex goal usage when the active Codex environment exposes goal tools. The skill starts or reuses an issue-scoped `create_goal` objective, records the latest `get_goal` usage snapshot and available estimate-style model profile metadata under the issue run directory as `codex-token-usage.json`, and includes a concise `token-usage` section in the original issue completion audit comment published during `/prs finish` with `prs audit publish --issue <number> --file <path> --section <name>`. Model metadata mirrors `prs issue estimate` profile formatting, such as `standard (gpt-5.4-mini, medium thinking)` for the default implementer profile or `premium (gpt-5.5, high thinking)` for premium-profile work. If usage or model metadata is unavailable, the workflow records that status without blocking verification, commit, push, or PR creation.
