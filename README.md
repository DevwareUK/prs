...
Codex-first issue completion does not require a configured text provider just to finish. `prs issue finalize <number>`, local `prs issue <number>`, and unattended `prs issue <number> --jdi` use provider-generated commit and PR text when the provider is available, and deterministic fallback text otherwise.
