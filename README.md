...
`prs issue refine <number>` is the guided issue-refinement flow for rough or non-technical tickets. It uses the GitHub issue comments as the refinement conversation, asks every currently blocking high-value question needed to understand the user's intention and likely knock-on effects, and does not publish a partial specification or plan while important answers are still missing. Once refinement is settled, prs keeps the issue body as concise summary/context and publishes the source-of-truth artifacts as managed comments: `<!-- prs:issue-spec -->` for the settled specification and `<!-- prs:issue-plan -->` for the implementation plan. It then adds a final confidence comment confirming the issue is ready for development.

`prs tool issue ready <issue-number> [--all] --json` blocks until both managed issue-refinement artifacts are present. If either the specification or plan comment is missing, the JSON result is `status: "blocked"` and points back to `prs issue refine <number>` instead of starting implementation.
