...
| `/prs pr <pr-number> review` | Prepares the live PR checkout plus repo-aware review context for the active Codex session, then writes a consolidated report under `.prs/runs` without editing, committing, pushing, or posting to GitHub. |
...
`/prs pr <pr-number> review` uses `prs tool pr review <pr-number> --json` to check out/sync the PR head, collect linked issues, changed files, diff, checks, issue comments, review comments, metadata, and a focused review prompt, then lets the active Codex session write `.prs/runs/<timestamp>-pr-<number>-review/codex-pr-review.md`. This first version is report-only: it does not edit code, commit, push, resolve comments, or post to GitHub.
