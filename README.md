...
For `prs-create`, the active agent uses Superpowers brainstorming to settle requirements and write and self-review the specification. It then uses Superpowers writing-plans to write and self-review the implementation plan and prepares the exact issue draft or linked set, without pausing for intermediate artifact approvals. Both files are required even for bounded work and stay under `.prs/runs`; the PRS locality rules override Superpowers document-path and commit defaults. Missing required skills block the workflow with a next action.

The agent presents the exact target/content, specification and plan together as one complete packet. One explicit approval accepts both artifacts and authorizes creating or reusing those displayed issues and publishing both managed comments. Design approval alone does not authorize the GitHub write. A reply that asks a question, qualifies approval or changes scope, content or target is not approval: the agent updates every affected artifact, presents the complete revised packet and asks for one fresh approval. The creation step uses both files:
